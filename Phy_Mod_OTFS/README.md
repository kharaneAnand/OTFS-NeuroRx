# OTFS Modulation
[![Github](https://img.shields.io/badge/Github-2.1.19-grey)](https://github.com/whatshow/Phy_Mod_OTFS) [![PyPi](https://img.shields.io/badge/PyPi-2.1.19-blue)](https://pypi.org/project/whatshow-phy-mod-otfs/) [![MathWorks](https://img.shields.io/badge/MathWorks-2.1.19-red)](https://mathworks.com/matlabcentral/fileexchange/161136-whatshow_phy_mod_otfs) [![996.icu](https://img.shields.io/badge/link-996.icu-red.svg)](https://996.icu)
...
> H. Chang, G. Pang, B. Vucetic and W. Hardjawana, "Neural OTFS Receiver Without Training Requirements," in IEEE Wireless Communications Letters, vol. 14, no. 11, pp. 3824-3828, Nov. 2025, doi: 10.1109/LWC.2025.3606969. keywords: {Symbols;Training;Receivers;Delays;Channel estimation;Doppler shift;Indexes;Complexity theory;Artificial neural networks;Vectors;Channel estimation;OTFS;symbol detection;untrained neural network},
...
* OTFS: this class provides the entire process of OTFS from Tx to Rx
    ...
    * setChannel(): this methods has multiple kinds of inputs.<br>
        ...
        * set a random channel (overwritten the channel setting; use Rayleigh fading if not select channel model)
            ...
            `@isSameDD(opt)`: use the same delay or doppler<br>
            ```c, matlab, python
    ...
    * passChannel()<br>
        `@No`: linear noise power (a scalar) or a given noise vector. [1]: the same noise power for all, [(batch_size), 1]: the same noise for all data in the same batch, [n] & [batch_size, n]: manuall defined noise(if you use batched data)
        ```c, matlab, python
        otfs.passChannel(No);
        ```
